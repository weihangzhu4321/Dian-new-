...
int save_file(const char *filename);  //文本写回文件，成功返回0,失败返回-1（errno已置位）
...
int editor(const char *filename)   
{
    ...
    while(1){
        ...
        ch = getch();
        if(ch == 17 || ch == 3)   //17为Ctrl+Q的ASCII码，3为Ctrl+C的ASCII码（保险），退出
        {
        ...
        else if(ch == 19)         //19为Crtl+S的ASCII码，保存文件
        {
          ...
          {
            char why[80];         //写入失败，显示原因
            snprintf(why, sizeof(why), "保存失败：%s", strerror(errno));
            show_message(why);
          }
          asking = 0;
        }
...
/*文本写回文件，成功返回0,失败返回-1（errno已置位）*/
int save_file(const char *filename)
{
    FILE *fp = fopen(filename, "wb");   //二进制写入，换行自己补（同load_file）
    int i;
    ...
    {
      size_t len = strlen(line_content[i]);
      
      if(line_count == 1 && len == 0)   //空文档（line_count至少为1,不然load_file返回错误）
        break;
      
      if(len > 0 && fwrite(line_content[i], 1, len, fp) != len)   //写入失败
      {
        fclose(fp);
        return -1;
      }
